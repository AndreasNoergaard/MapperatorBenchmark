...
namespace BenchmarkFastMapper
{
    public class Program
    {
        ...
        public static void TestSimpleTypes()
        {
            Console.WriteLine("Test 1 : Simple Types");
            Console.WriteLine("Competitors : FastMapper, Value Injecter, AutoMapper");

            var foo = GetFoo();

            #region Create Mapping & Caching
        ...
        public static void TestComplexTypes()
        {
            Console.WriteLine();
            Console.WriteLine();

            Console.WriteLine("Test 2 : Complex Types");
            Console.WriteLine("Competitors : Handwriting Mapper, FastMapper, AutoMapper");
        ...
        private static void Test(Customer item, int iterations)
        {
            ...
            TestMapperator(item, iterations);
        ...
        public static CustomerDTO MapCustomerNative(Customer customer)
        {
            CustomerDTO dto = new CustomerDTO();
            ...
            dto.Address = new Address() { Id = customer.Address.Id, Street = customer.Address.Street, Country = customer.Address.Country, City = customer.Address.City };

            dto.HomeAddress = new AddressDTO() { Id = customer.HomeAddress.Id, Country = customer.HomeAddress.Country, City = customer.HomeAddress.City };

            dto.Addresses = new AddressDTO[customer.Addresses.Length];
            for (int i = 0; i < customer.Addresses.Length; i++)
            {
                dto.Addresses[i] = new AddressDTO() { Id = customer.Addresses[i].Id, Country = customer.Addresses[i].Country, City = customer.Addresses[i].City };
            }

            dto.WorkAddresses = new List<AddressDTO>();
            foreach (var workAddress in customer.WorkAddresses)
            {
                dto.WorkAddresses.Add(new AddressDTO() { Id = workAddress.Id, Country = workAddress.Country, City = workAddress.City });
            }
            return dto;
        ...
        {
            Console.WriteLine("FastMapper:\t\t" + Loop<TSrc>(item, get =>
            {
            ...
            where TDest : class, new()
        {
            ...
            Console.WriteLine("AutoMapper:\t\t" + Loop<TSrc>(item, get =>
            {
                Mapper.Map<TSrc, TDest>(get);
            }, iterations));
        }

        private static void TestMapperator(Customer item, int iterations)
        {
            Console.WriteLine("Mapperator:\t\t" + Loop<Customer>(item, get =>
            {
                CustomerToCustomerDTOMapper.Instance.Map(item);
            }, iterations));
        }
